...
#include "zalpinebotcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zALPB ought to be enough for anybody." - Bill Gates, 2017
    ui->zALPBpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzALPBSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzALPB->setEnabled(false);
        ui->pushButtonMintzALPB->setToolTip(tr("zALPB is currently disabled due to maintenance."));

        ui->pushButtonSpendzALPB->setEnabled(false);
        ui->pushButtonSpendzALPB->setToolTip(tr("zALPB is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zALPBpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzALPB_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zALPB is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zALPB...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zALPB in ") +
...
void PrivacyDialog::on_pushButtonSpendzALPB_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zALPB is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zALPB
        sendzALPB();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zALPB
    sendzALPB();
}

void PrivacyDialog::on_pushButtonZAlpineBotControl_clicked()
{
    ZAlpineBotControlDialog* zALPBControl = new ZAlpineBotControlDialog(this);
    zALPBControl->setModel(walletModel);
    zALPBControl->exec();
}

void PrivacyDialog::setZAlpineBotControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzALPBSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzALPB()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid AlpineBot Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zALPBpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zALPB is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ALPINEBOT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zALPB</b>";
    ...
    // use mints from zALPB selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZAlpineBotControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZAlpineBotControlDialog::GetSelectedMints();
    }
    ...
    // Clear zalpinebot selector in case it was used
    ZAlpineBotControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zALPB Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " AlpineBot, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zALPB Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zALPB </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zALPB "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zALPB "));
    ui->labelzALPBAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzALPBSyncStatus->setVisible(fShow);
}
