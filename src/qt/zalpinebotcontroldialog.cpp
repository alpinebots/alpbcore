...
#include "ui_zalpinebotcontroldialog.h"
...
std::list<std::string> ZAlpineBotControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZAlpineBotControlDialog::listMints;

ZAlpineBotControlDialog::ZAlpineBotControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZAlpineBotControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZAlpineBotControlDialog::~ZAlpineBotControlDialog()
{
    delete ui;
}

void ZAlpineBotControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAlpineBotControlDialog::updateList()
{
...
void ZAlpineBotControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAlpineBotControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZAlpineBot_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAlpineBotControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZAlpineBotControlDialog::GetSelectedMints()
{
...
void ZAlpineBotControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
